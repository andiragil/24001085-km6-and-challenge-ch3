...
function displayCars(cars) {
  ...
  if (cars.length === 0) {
    carsContainer.innerHTML = `<div class="alert alert-danger" role="alert">
      Mobil tidak ditemukan
    </div>`;
    return;
  }
...
async function listCar() {
  ...
  const param = new URLSearchParams(list);
  const manufacture = param.get("manufacture");
  if (manufacture) {
    const result = await carResult(manufacture);
